...
/*
 * esnregress.cpp
 * Echo State Network in Arrayfire, uses least squares to compute output weights for regression.
 * See Mantas esn.py example and PracticalESN.pdf (10.1007/978-3-642-35289-8-36)
 * Examples:
 *	esnregress --reg=-8 --wscale=0.5 --trainlen=2000 --testlen=2000 --dump --input=data/mackeyglass_t17.txt >/tmp/d
 *	esnregress --wscale=0.1 --radius=1.1 --reg=-3 -leakage=0.91 --trainlen=8000 --testlen=40 --input=data/laser -d >/tmp/d
 */
...
long noutputs = 1;	// number of outputs  NOT USED???
...
timer::start();
	...
	for ( int t=0 ; t<testlen ; t++ ) {
    	...
    	// run through the output network to get prediction y
		// this is really just a dot product, both args are 1d)
    	array y = matmul(Wout, join(0,u,x));

#ifdef GENERATIVE
    	// free running generative mode, next input is our current prediction
		// (need to prepend the constant 1)
    	u = join(0,constant(1.0,1,f64), y);
#else
    	// for 1 step ahead prediction, set u to actual value
    	u = transpose(data1(trainlen+t+1,span));
#endif // GENERATIVE

		// difference between predicted and actual data
		// (use sum to convert to double)
		double d = sum<double>(y - data(trainlen+t+1,0));
		mse +=  d*d;
		// to dump actual and predicted free running values for plotting
		if ( dump ) {
			double predict = sum<double>(y);
			double actual = sum<double>(data(trainlen+t+1,0));
			printf("%f %f\n",predict,actual);
		}
	}
